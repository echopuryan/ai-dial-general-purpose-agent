...
class PythonCodeInterpreterTool(BaseTool):
    ...
    def __init__(
        ...
        if self._code_execute_tool is None:
            raise ValueError(f"Tool with name '{tool_name}' not found in MCP tool models")
    ...
    async def create(
        ...
        return cls(mcp_client=client, mcp_tool_models=mcp_tool_models, tool_name=tool_name, dial_endpoint=dial_endpoint)
